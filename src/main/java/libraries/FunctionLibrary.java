   ...
   public static void launchBrowserDeleteIt(String URL,String Desc,String browserName)
   {
...

      {
         ...
         if(browserName.equalsIgnoreCase("firefox")) {
            ...
            ObjDriver.manage().timeouts().implicitlyWait(90, TimeUnit.SECONDS);
...
//       }
      ...
      if(!Expected_Value.matches("NA"))
      {
         ...
         else
         {
            ...
            if(Actual_Text_Val.equalsIgnoreCase(Expected_Value))
            {
               if(Desc!="")
               {
                  ReportLibrary.Add_Step(ReportLibrary.Test_Step_Number, Desc+ ": Actual Value :" + Actual_Text_Val + ", Expected Value : " + Expected_Value, LogStatus.PASS, screenShot);
               }
            }
            else
            {
   ...
   public static void setCheckBox(String CheckBox_Pro,String CheckBox_Val,String Desc)
   {
      ...
      if(!CheckBox_Val.toString().matches("NA"))
      {
         ...
         Wait_For_Object.until(ExpectedConditions.visibilityOf(CheckBox));
         Wait_For_Object.until(ExpectedConditions.elementToBeClickable(CheckBox));
         if(CheckBox == null)
         {
            //ReportLibrary.Add_Step(ReportLibrary.Test_Step_Number, Desc, LogStatus.FAIL, true);Exception_Content();
         }
      ...
      if(!Event.matches("NA"))
      {
         ...
         try
         {
            ...
            if(Object == null)
            {
               if(Desc!="")
               {
                  //ReportLibrary.Add_Step(ReportLibrary.Test_Step_Number, "Expected:" +Desc, LogStatus.FAIL, true);Exception_Content();
               }
            }
            else
            {
   ...
   public static void verifyWebTableData(String WebElement_Pro,String ExpectedText,String Desc) throws Exception
   {
   ...
   public static void verifyWebTableHeaders(String WebElement_Pro,String WebTable_Headers,String Desc) throws Exception
   {
...
//    Wait_For_Div.until(ExpectedConditions.invisibilityOfElementLocated(ObjDriver.findElements(By.xpath("//div[contains(@id,'loadingPanelCursorDiv')]")).size() > 0));
   ...
   public static boolean verifyWebTableCellData(String WebElement_Pro,int rowNumber, int columnNumber, String ExpectedText,String Desc)
   {
      ...
      if(!ExpectedText.matches("NA"))
      {
         ...
         String text = repalceStrings(ExpectedText);
         WebElement elementWebTable = Get_Element(WebElement_Pro);
        rowElement = elementWebTable.findElements(By.tagName("tr")).get(rowNumber);
         ...
         {
            if(text.equalsIgnoreCase(actualCellElement.getText()))
            {
            ...
            else
            {
               ReportLibrary.Add_Step(ReportLibrary.Test_Step_Number, Desc + " : "+ ExpectedText + ".<br>"
                     +"Acutal is: " +actualCellElement.getText(), LogStatus.FAIL, true);
               Exception_Content();
         ...
         {
            ReportLibrary.Add_Step(ReportLibrary.Test_Step_Number, "Web table cell is not found with row "
   ...
   public static int getRowNumberFromWebTable(String WebElement_Pro,String rowText,String Desc)
   {
      ...
      if(!WebElement_Pro.matches("NA"))
      {
         ...
         int noOfColumns = elementWebTable.findElements(By.tagName("tr")).get(2).findElements(By.tagName("td")).size();
         if(elementWebTable!=null)
         {
            ...
            for(int iterator = 0; iterator< noOfRows;iterator++)
            {
               ...
               for(int iterator1 = 0; iterator1< noOfColumns;iterator1++)
               {
                  ...
                  if(rowText.equalsIgnoreCase(actualCellElement.getText()))
                  {
                     rowNumber = iterator;
 ...
 //Function for Scroll Down to Element
...
//Function for Scroll Up to Element
         ...
         //Random Number generator upto 5digits
